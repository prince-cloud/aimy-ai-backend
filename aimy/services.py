...
# Local imports
from .models import Document, DocumentChunk, ProcessingQueue, ChatSession, ChatMessage, GenericFile, GenericFileChunk
...
class DocumentProcessingService:
    ...
    def process_generic_file(self, generic_file: GenericFile) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully created Redis vector store for generic file {generic_file.id}")
            except Exception as redis_error:
                ...
                try:
                    ...
                    logger.info(f"Successfully created Chroma vector store for generic file {generic_file.id}")
                except Exception as chroma_error:
                    logger.error(f"Chroma vector store creation failed: {str(chroma_error)}")
                    raise Exception(f"Vector store creation error (both Redis and Chroma failed): {str(redis_error)} -> {str(chroma_error)}")
    ...
    def _load_generic_file(self, generic_file: GenericFile) -> List[LangchainDocument]:
        ...
        if not generic_file.is_file_accessible():
            raise Exception(f"File is not accessible for generic file {generic_file.id}")
        ...
        try:
            ...
            docs = loader.load()
            logger.info(f"Successfully loaded {len(docs)} documents from generic file {generic_file.id}")
            return docs
...
class ChatService:
    ...
    def ask_question(
        ...
        try:
            ...
            if document_id:
                return self._handle_document_specific_question(question, session, user_id, document_id, max_results, temperature)
            
            # Priority 2: If session_id is provided, prioritize that session's document
            if session_id and session.document:
                return self._handle_session_specific_question(question, session, user_id, session.document.id, max_results, temperature)
            
            # Priority 3: Use generic files (admin-uploaded files) - similar to document-specific approach
            return self._handle_generic_files_question(question, session, user_id, max_results, temperature)
    ...
    def _handle_document_specific_question(
        ...
        try:
            ...
            if not vector_stores:
                return self._create_error_response(f"No document with ID {document_id} available for search")
            ...
            if not relevant_docs:
                return self._create_error_response(f"No relevant information found in the specified document")

            # Generate response
            response = self._generate_response(question, relevant_docs, session, temperature)
    ...
    def _handle_session_specific_question(
        ...
        try:
            ...
            vector_stores = self._get_vector_stores(user_id, document_id)
            if not vector_stores:
                return self._create_error_response(f"No document available for this session")

            # Search for relevant content in the session's document
            relevant_docs = self._search_documents(question, vector_stores, max_results)
            
            if not relevant_docs:
                return self._create_error_response(f"No relevant information found in the session's document")
    ...
    def _handle_generic_files_question(
        ...
        try:
            ...
            if not vector_stores:
                ...
                return self._handle_general_question(question, session, user_id, max_results, temperature)
            ...
            if not relevant_docs or self._is_low_relevance_response(relevant_docs, question):
    ...
    def _is_low_relevance_response(self, relevant_docs: List[Dict[str, Any]], question: str) -> bool:
    ...
    def _is_knust_specific_question(self, question: str) -> bool:
        ...
        knust_specific_keywords = [
            'admission', 'apply', 'application', 'requirements', 'cut-off', 'cutoff',
            'colleges', 'faculty', 'school', 'program', 'course', 'degree',
            'contact', 'phone', 'email', 'address', 'location', 'campus',
            'student life', 'accommodation', 'fees', 'tuition', 'scholarship',
            'research', 'graduate', 'postgraduate', 'masters', 'phd'
        ]
        
        question_lower = question.lower()
        return any(keyword in question_lower for keyword in knust_specific_keywords)
    
    def _handle_knust_question(self, question: str, session: ChatSession, user_id: int) -> Dict[str, Any]:
    ...
    def _get_vector_stores(
        ...
        if document_id:
            ...
            documents = Document.objects.filter(id=document_id, user_id=user_id, is_processed=True)
    ...
    def _load_document_vector_store(self, doc: Document):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Both Redis and Chroma failed for document {doc.id}: {str(e)} -> {str(chroma_error)}")
                return None
    ...
    def _load_generic_file_vector_store(self, generic_file: GenericFile):
        ...
        try:
            ...
            logger.warning(f"Redis vector store failed for generic file {generic_file.id}: {str(e)}")
            # Try Chroma as fallback
            try:
                vector_store = Chroma(
                    embedding_function=self.embeddings,
                ...
                logger.info(f"Loaded Chroma vector store for generic file {generic_file.id}")
                return vector_store
            except Exception as chroma_error:
                logger.error(f"Both Redis and Chroma failed for generic file {generic_file.id}: {str(e)} -> {str(chroma_error)}")
                return None
            ...
            try:
                ...
                if hasattr(vector_store, "similarity_search_with_score"):
                    results = vector_store.similarity_search_with_score(
                        question, k=max_results * 2  # Get more results for hybrid search
                    )
                else:
                    # Fallback for Chroma
    ...
    def _search_database_directly(self, question: str, max_results: int) -> List[Dict[str, Any]]:
            ...
            for course_code in course_matches:
                ...
                for chunk in chunks:
                    ...
                    if chunk.chunk_index > 0:
                        ...
                        if prev_chunk and ('MORNING SESSION' in prev_chunk.content or 'EVENING SESSION' in prev_chunk.content):
                    ...
                    if next_chunk and ('AUGUST' in next_chunk.content or 'MONDAY' in next_chunk.content):
